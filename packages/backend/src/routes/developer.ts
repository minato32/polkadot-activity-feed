...
export function registerDeveloperRoutes(app: FastifyInstance): void {
  /**
   * GET /api/v1/events — same as /api/events but authenticated via X-API-Key.
   * Rate limits are enforced by the requireApiKey middleware per tier.
   */
  app.get<{ Querystring: V1EventsQuerystring }>(
    "/api/v1/events",
